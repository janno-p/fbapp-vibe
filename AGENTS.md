...
- If you edit under `src/`, `src/modules/<name>/`, or `tests/`, read the nearest `CLAUDE.md` first. Repo-wide guidance also lives in `.claude/CLAUDE.md`.
...
- Task work is spec-first in `.claude/tasks/`: copy `TEMPLATE.md`, fill acceptance criteria, move `open -> in-progress -> done`, and fill `## Outcome` before closing.
