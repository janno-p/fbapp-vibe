...
source: .claude/tasks/done/0016-batch-seeding-inserts.md
source_id: 0016
source_status: open
source_title: Batch player and group membership inserts during tournament seeding
status: open
type: chore
adrs: []
refs: [0005]
...
Replaced the per-player loop and per-membership inline calls with two UNNEST-based bulk upserts:

- Step 3: collects `(tournament_id, external_id, name, team_id)` parallel arrays across all squads, then executes a single `INSERT ... SELECT $1, unnest($2::text[]), unnest($3::text[]), unnest($4::bigint[]) ON CONFLICT DO UPDATE` query. Skipped entirely if no players.
- Step 4: collects `(group_id, team_id)` pairs into a `HashSet` (deduplicates within the loop), then executes a single `INSERT ... SELECT unnest($1::bigint[]), unnest($2::bigint[]) ON CONFLICT DO NOTHING` after the match loop. Skipped entirely if no group-stage matches.
- Deleted `upsert_player` and `upsert_group_membership` private functions (no other callers).
- Removed unused `Player as ApiPlayer` import.
- `seed_is_idempotent` test passes unchanged.
